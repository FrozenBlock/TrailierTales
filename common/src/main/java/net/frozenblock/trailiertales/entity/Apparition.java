...
public class Apparition extends Monster implements RangedAttackMob {
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, EntitySpawnReason reason, SpawnGroupData entityData) {
		this.getBrain().setMemoryWithExpiry(TTMemoryModuleTypes.AID_COOLDOWN.get(), Unit.INSTANCE, 100L);
	...
	public float getWalkTargetValue(BlockPos pos) {
		...
		if (this.frozenLib$getAttached(TTAttachmentTypes.ENTITY_COFFIN_DATA) != null && level instanceof ServerLevel serverLevel) {
	...
	protected SoundEvent getAmbientSound() {
		return TTSounds.APPARITION_IDLE.get();
	}

	@Override
	protected SoundEvent getHurtSound(DamageSource source) {
		return TTSounds.APPARITION_HURT.get();
	}

	@Override
	protected SoundEvent getDeathSound() {
		return TTSounds.APPARITION_DEATH.get();
	}
	...
	public boolean isAiding() {
		return this.getBrain().hasMemoryValue(TTMemoryModuleTypes.AIDING_TIME.get());
	}
	...
	public void performRangedAttack(LivingEntity target, float pullProgress) {
		...
		this.playSound(TTSounds.APPARITION_THROW.get(), 1F, 0.4F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
